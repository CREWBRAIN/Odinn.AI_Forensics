...
def convert_single_pdf(
    ...
    if fname.lower().endswith('.pdf'):
        ...
        pages, toc = process_single_image(image, model_lst, langs, batch_multiplier)
        out_meta.update({
            "toc": toc,
            "pages": 1,
        })
...
def process_single_image(image: Image.Image, model_lst: List, langs: List[str], batch_multiplier: int) -> Tuple[List, Dict]:
    # Implement the logic to process a single image
    # This should mimic the behavior of processing a single page from a PDF
    page = Page(0)
    page.image = image
    pages = [page]
    toc = {}  # No table of contents for a single image

    # Perform OCR and other processing steps as needed
    # You may need to adapt some of the functions used in convert_single_pdf to work with a single image
